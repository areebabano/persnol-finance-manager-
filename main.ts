...
import { addBill, getBill, updateBill, deleteBill, bills } from "./bill.js";
...
import { RecurrenceInterval, RecurringTransactionType } from "./types.js";
import {
...
const handleAddBudget = async (): Promise<void> => {
  try {
    ...
    const budget = addBudget(
      ...
      parseFloat(budgetLimit.limit)
    );
    console.log(
      chalk.blueBright(`\nBudget Added Successfully!
  You've set a budget of Rs.${chalk.bold(budgetLimit.limit)} for ${chalk.bold(
        budgetCategory.category
      )}.\n`)
    );
  } catch (error) {
...
const handleUpdateBudget = async (): Promise<void> => {
  try {
    ...
    const budget = updateBudget(
      ...
      parseFloat(updatedBudget.limit)
    );
    if (budget) {
      console.log(
        chalk.green(
...
const handleViewBudgets = async (): Promise<void> => {
  const budgets = await getBudget();
...
const handleAddExpense = async (): Promise<void> => {
  try {
    ...
    await addExpense(
      ...
      new Date(expenseDate.date),
      expenseAmount.amount
    );

    console.log(
      chalk.blueBright(`\nExpense Added Successfully!
      You've set a expense of Rs.${chalk.bold(
        parseFloat(expenseAmount.amount)
      )} for ${chalk.bold(expenseCategory.category)}.\n`)
    );
  } catch (error) {
  ...
  try {
    ...
    if (expense) {
      console.log(
        ...
        You've set a expense of Rs.${chalk.bold(
          parseFloat(updatedExpense.amount)
        )} for ${chalk.bold(updatedExpense.category)}.\n`)
      );
    } else {
...
const handleViewExpenses = async (): Promise<void> => {
  const expenses = await getExpense();
  console.log(chalk.magenta("Current Expense:"));
  expenses.forEach((expense) => {
    console.log(
      chalk.cyan(
        `- ${expense.category} - ${expense.date}: Rs.${expense.amount}`
      )
    );
  });
};
...
const handleAddIncome = async (): Promise<void> => {
  try {
    ...
    await addIncome(
      incomeId,
      incomeSource.source,
      new Date(incomeDate.date),
      parseFloat(incomeAmount.amount)
    );

    console.log(
      chalk.blueBright(
        `\nIncome tracked and recorded successfully from '${
          incomeSource.source
        }' on ${chalk.bold(incomeDate.date)}. Amount: Rs.${chalk.bold(
          parseFloat(incomeAmount.amount)
        )}.\n`
      )
    );
...
const handleUpdateIncome = async (): Promise<void> => {
  try {
    ...
    const income = updateIncome(
      ...
      updatedIncome.date,
      parseFloat(updatedIncome.amount)
    );
    if (income) {
      console.log(
        chalk.green(
          `\nIncome tracked successfully updated! and recorded successfully from '${
            updatedIncome.source
          }' on ${chalk.bold(updatedIncome.date)}. Amount: Rs.${chalk.bold(
            parseFloat(updatedIncome.amount)
          )}.\n`
        )
      );
...
const handleViewIncome = async (): Promise<void> => {
  const incomes = await getIncome();
  console.log(chalk.magenta("Current Income:"));
  incomes.forEach((income) => {
    console.log(
      chalk.cyan(`- ${income.source} - ${income.date}: Rs.${income.amount}`)
    );
  });
};
...
const handleAddSavings = async (): Promise<void> => {
  try {
    ...
    const savingId = uuid();
    await addSavingGoals(
      savingId,
      savingGoals.goal,
      parseFloat(savingAmount.amount)
    );
    console.log(
      chalk.blueBright(
        `\nSavings goal '${
          savingGoals.goal
        }' successfully! added with a target amount of Rs.${chalk.bold(
          parseFloat(savingAmount.amount)
        )}.\n`
      )
    );
...
const handleUpdateSavings = async (): Promise<void> => {
  try {
    ...
    const saving = updateSavingGoals(
      savingId,
      updatedSavingGoal.goal,
      parseFloat(updatedSavingGoal.amount)
    );
    if (saving) {
      console.log(
        chalk.green(
          `\nSavings goal '${
            updatedSavingGoal.goal
          }' successfully updated! with a target amount of Rs.${chalk.bold(
            parseFloat(updatedSavingGoal.amount)
          )}.\n`
        )
      );
...
const handleDeleteSavings = async (): Promise<void> => {
  try {
    const savingGoals = await getSavingGoals();
    const savingChoices = savingGoals.map((savingGoal) => ({
      name: `${savingGoal.goals} - Rs.${savingGoal.amount}`,
...
const handleViewSavings = async (): Promise<void> => {
  const savingGoals = await getSavingGoals();
...
const handleAddInvestment = async (): Promise<void> => {
  try {
    ...
    console.log(
      chalk.blueBright(
        `\nInvestment '${
          investmentName.name
        }' successfully recorded on ${chalk.bold(
          investmentDate.date
        )} with an amount of Rs.${chalk.bold(
          parseFloat(investmentAmount.amount)
        )}.\n`
      )
    );
...
const handleUpdateInvestment = async (): Promise<void> => {
  try {
    ...
    const investment = updateInvestment(
      investmentId,
      updatedInvestment.name,
      updatedInvestment.date,
      parseFloat(updatedInvestment.amount)
    );
    if (investment) {
      console.log(
        chalk.green(
          ...
          }' successfully updated! recorded on ${chalk.bold(
            updatedInvestment.date
          )} with an amount of Rs.${chalk.bold(
            parseFloat(updatedInvestment.amount)
          )}.\n`
...
const handleDeleteInvestment = async (): Promise<void> => {
  try {
    const investments = await getInvestment();
    const investmentChoices = investments.map((investment) => ({
      name: `${investment.name} - Rs.${investment.amount}`,
...
const handleViewInvestment = async (): Promise<void> => {
  const investments = await getInvestment();
...
const handleAddBill = async (): Promise<void> => {
  try {
    ...
    await addBill(
      ...
      billName.bill,
      billAmount.amount,
      new Date(billDueDate.dueDate)
    );
    console.log(
      chalk.blueBright(
        `\nBill reminder '${billName.bill}' successfully added! Rs${
          billAmount.amount
        } with a due date of ${chalk.bold(billDueDate.dueDate)}.\n`
      )
    );
  } catch (error) {
...
const handleDeleteBill = async (): Promise<void> => {
  try {
    const bills = await getBill();
    const billChoices = bills.map((bill) => ({
      name: `${bill.name} - Rs.${bill.amount}`,
...
const handleViewBill = async (): Promise<void> => {
  const bills = await getBill();
...
const handleInsights = async (): Promise<void> => {
  try {
  ...
  try {
    const accountDetails = await inquirer.prompt([
      { name: "name", type: "input", message: "Enter account name:" },
      {
        ...
        message: chalk.magentaBright(
          "Enter account type (savings/checking/credit):"
        ),
      },
      {
        name: "balance",
    ...
    console.log(
      chalk.blueBright(
        `\nAccount '${
          accountDetails.name
        }' successfully added with an initial balance of Rs.${chalk.bold(
          parseFloat(accountDetails.balance)
        )}.\n`
      )
    );
  ...
  try {
    ...
    const updatedDetails = await inquirer.prompt([
      { name: "name", type: "input", message: "Enter new account name:" },
      {
        ...
        type: "input",
        message: chalk.magentaBright(
          "Enter new account type (savings/checking/credit):"
        ),
      },
      {
        name: "balance",
    ...
    updateAccount(
      ...
      updatedDetails.name,
      updatedDetails.type,
      parseFloat(updatedDetails.balance)
    );
    console.log(
      chalk.green(
        `\nAccount '${
          updatedDetails.name
        }' successfully updated! with an initial balance of Rs.${chalk.bold(
          parseFloat(updatedDetails.balance)
        )}.\n`
      )
    );
...
const handleViewAccount = async (): Promise<void> => {
  const accounts = await getAccount();
...
const handleSecurity = async (): Promise<void> => {
  try {
...
const handleAddRecurringTransaction = async (): Promise<void> => {
  try {
    const categories = Object.values(Categories);
    ...
    const newTransactions: RecurringTransactionType = {
      id: uuid(),
      type: transactionDetails.type,
      category: transactionDetails.category as Categories,
      amount: parseFloat(transactionDetails.amount),
      frequency: transactionDetails.frequency as RecurrenceInterval,
      nextDate: new Date(transactionDetails.nextDate),
    };
    await addRecurringTransaction(
      newTransactions.id,
      newTransactions.type,
      newTransactions.nextDate,
      newTransactions.amount,
      newTransactions.category,
      newTransactions.frequency
    );
...
(async (): Promise<void> => {
  try {
    ...
    console.log(
      chalk.yellow(
        "------------___________| SIGN-UP PERSNOL FINANCE MANAGER |___________----------"
      )
    );
    ...
    const isAuthenticate = await userAuthentication(
      answer.userName,
      answer.password,
      answer.email
    );
